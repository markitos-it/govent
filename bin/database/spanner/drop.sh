...
echo "🗑️  Deleting database 'govent' from Spanner..."
...
docker run --rm --network container:go-events-spanner google/cloud-sdk:alpine /bin/sh -c "
  ...
  gcloud spanner databases delete govent --instance=local-instance --quiet 2>/dev/null || echo '   ⚠️ Database does not exist or has already been deleted.'
"
